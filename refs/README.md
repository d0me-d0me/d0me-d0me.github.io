...
1. `content.js` の `volumes[]` に1項目追加(`domain` で大項目に紐付け、`slug` が `sheets/<slug>.html` の URL anchor 兼永続キー、`tags` で横断分類)。新しい大項目が要れば `domains[]` に追加。
2. `sheets/template.html` を複製して本文を書き、`sheets/<slug>.html` として置く。
3. `status` を `"ready"` にすると索引が自動でリンク付きになり、件数も更新される。

資格の増減は `content.js` の `certs[]` と `assets/badges/<name>.png` を編集するだけ。
